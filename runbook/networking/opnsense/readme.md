...
This isn't meant to be a definitive guide on setting up opnSense, instead it's meant to make note of common gotchas, pitfalls, etc., you may encounter whether you're setting up your first firewall or if you're experienced with them and ard migrating from a tool like pfSense. Also, there are probably better ways to do certain things, 


## Basic Setup 

Ideally you would setup a device with multiple LAN ports, as this would allow enable you to designate one for management only and/or be a fallback position if you have network issues and can't access the device from the main network. I.e., you'd have WAN(s), the management only LAN and then the LAN(s) for your network. Here is how I set things up:

### Initial Interfaces

* You'll may to configure these ones from the command line, so plug a monitor and a keyboard into your opnSense instance.
...
* Once I had LAN port designated with the baseline 192.168.1.1 IP I connected a Raspberry Pi to it that was running Tailscale and was advertising the subnet 192.168.1.1/24. This gave me my entry point. I would suggest not changing this IP as it can break things and you'll find yourself not able to access opnSense without directly accessing it from the device and possibly needing to reset the firewall to factory settings. Also: if you try to change it opnSense strongly warns you not to, so, why bother with the trouble? 
