...
# Script for retrieving and analyzing Technitium data for high rates of
...
def build_dashboard_data_url():
    ...
    logger.info('API connection URL created for: %s',
                DNS_ID)
...
def prepare_payload(data: dict, failure_ratio, refusal_ratio):
    ...
    logger.info('Technitium DNS payload ready: %s',
                payload)
...
def main():
    ...
    while True:
        ...
        if status != 'ok':
            logger.info('Connection issue, status is: %s, going to sleep for %s seconds',  # noqa: E501
                        status,
                        SLEEP_DURATION)
